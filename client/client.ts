...
class UpdateEmployee {
  ...
  constructor(fields: { id: number; grade: number }) {
    (this.id = fields.id), (this.grade = fields.grade);
  }
}
class Amount {
  amount: number;
...
async function client() {
  ...
  const programId = new solanaweb3.PublicKey(
    "8f4tWJU1kVofUAJVey17yv5jif57GxsHbBTaRd2qmk8"
  );
  ...
  const private_key = [
    174, 43, 71, 196, 184, 1, 205, 37, 10, 122, 10, 118, 196, 190, 229, 119,
    109, 33, 9, 37, 61, 226, 49, 76, 144, 229, 61, 64, 60, 251, 55, 36, 173,
    238, 155, 56, 68, 233, 39, 130, 57, 23, 65, 178, 29, 43, 220, 91, 241, 170,
    35, 204, 36, 254, 33, 109, 101, 183, 229, 70, 215, 137, 37, 75,
  ];
  ...
  const lamportsEqualTo = await connection.getMinimumBalanceForRentExemption(
    EMPLOYEE_SIZE
  );

  // generating a public key for the PDA, the account that will hold the data sent to the blockchain
  const genpubkey = await solanaweb3.PublicKey.createWithSeed(
    feePayer.publicKey,
    "blihoiu8y",
    programId
  );
  ...
  const signature = await solanaweb3
    .sendAndConfirmTransaction(connection, transaction, [feePayer])
    .catch((err: any) => {
      console.log(err);
    });
...
async function addEmployee(
  ...
  let accounts = [
    {
      pubkey: new solanaweb3.PublicKey(
        "J5ibHQpSChTHXT9N8MWoqafNkdTpEfGrkJm2jqzdFbF"
      ),
      isSigner: false,
      isWritable: true,
    },
...
async function getEmployeeDetails() {
  ...
  const dataAccount = new solanaweb3.PublicKey(
    "J5ibHQpSChTHXT9N8MWoqafNkdTpEfGrkJm2jqzdFbF"
  );
  ...
  const deserializedDataEqualTo = borsh.deserializeUnchecked(
    employeeSchema,
    Employee,
    data.data
  );
...
async function transferTokens(numerbOfTokes: number) {
  ...
  const mint = new solanaweb3.PublicKey(
    "9ihniMP4GAvBgicTBUmDr4p3ik3eQLB89kSJ4a8MrKVA"
  );
  const feePayer = solanaweb3.Keypair.fromSecretKey(
    new Uint8Array(private_key)
  );
  ...
  let accounts = [
    {
      pubkey: new solanaweb3.PublicKey(
        "HgvWqfoXfihjQpViArDwaELHvmZXyZ5vkarwTeWpsxEA"
      ),
      isSigner: false,
      isWritable: true,
    },
...
async function addPet(name: string, age: number, breed: string) {
  ...
  const petSignature = await solanaweb3
    .sendAndConfirmTransaction(connection, petTransaction, [feePayer])
    .catch((err: any) => {
      console.log(err);
    });
